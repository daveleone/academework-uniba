<x-app-layout>
    ...
    <div class="py-12 flex flex-col items-center w-full text-lg">
        ...
        <div id="editDiv" class="flex flex-col hidden m-2.5 w-[30rem] p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <form action="{{ route('exercise.edit', ['id' => $exercise->id]) }}" method="POST" id="closed-Form">
                ...
                @foreach ($exercise->elements()->orderBy('position')->get() as $element)
                <div  id="{{'ansDiv' . $element->position}}" class="w-[17rem] my-[1rem]">
                    <label for="{{ 'exAnswer' . $element->position }}" id="{{'ansLab'.$element->position}}" class="flex mb-2 text-sm font-medium text-gray-900 dark:text-white">Answer {{ $element->position + 1 }}: </label>
            ...
            <div class="flex flex-row mt-[2rem]">
                <button type="submit" form="closed-Form" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Edit</button>
                <button onclick="addAnswer()" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Add answer</button>
                <button onclick="disableEdit()" class="py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Abort</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        for(let i = 1; i<answerNum.value; i++){
            ...
            deleteButtn.onclick = () => { deleteAnswer("ansDiv" + i, i); };
        ...
        function addAnswer(){
            ...
            const newAnswer = document.getElementById('ansDiv0').cloneNode(true);
