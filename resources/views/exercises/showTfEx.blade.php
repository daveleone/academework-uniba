<x-app-layout>
        ...
        <div style="background-color: white" id="showDiv">
           <p>{{ $exercise->description }}</p>
            ...
            <div>
                ...
                <input type="checkbox" id="{{ 'answer' . $element->position }}"
            ...
            <form action="{{ route('exercise.edit', ['id' => $exercise->id]) }}" method="POST" id="tf-Form">
                ...
                <div id="tf-FormDiv">
                    ...
                    <div  id="{{'questDiv' . $element->position}}">
                        ...
                        <input type="text" name="{{ 'question' . $element->position }}"
                            id="{{ 'question' . $element->position }}"
                            value="{{ $element->content }}" required
                        >
                        <input type="checkbox" id="{{ 'isTrue' . $element->position}}"
                            name="{{ 'isTrue' . $element->position}}" value="1"
    ...
    <script>
        ...
        for(let i = 1; i<questionNum.value; i++){
            ...
            deleteButtn.onclick = () => { deleteQuestion(questionDiv.id, i); };
