<x-modal name="edit-subject-{{ $subject->id }}">
    <div class="max-w-md mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <form class="px-8 py-6 space-y-6" action="{{ route('subject.edit') }}" method="POST">
            @method('PUT')
            @csrf
            <input type="hidden" name="subId" value="{{ $subject->id }}">
            <div>
                <label for="{{ 'subName'.$subject->id }}" class="block text-sm font-medium text-gray-700">
                    @lang('trad.Subject\'s name')
                </label>
            ...
            <div>
                <label for="{{ 'subDesc'.$subject->id }}" class="block text-sm font-medium text-gray-700">
                    @lang('trad.Subject\'s Description')
                </label>
                <div class="mt-1 relative rounded-md shadow-sm">
