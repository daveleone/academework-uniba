            ...
            <div>
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                    ...
                    <textarea id="{{ 'topDesc'.$topic->id }}" name="{{ 'topDesc'.$topic->id }}" rows="4"
                              class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md" dusk="edit-topic-description" placeholder="{{ $topic->description }}"></textarea>
                </div>
            </div>
            <div>
                <div class="flex items-center justify-between space-x-4">
                    <button type="submit" class="group w-1/2 inline-flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-300 ease-in-out transform hover:-translate-y-1" dusk="submit-edit-topic">
