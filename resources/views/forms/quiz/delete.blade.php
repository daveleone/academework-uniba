<x-modal name="delete-quiz-{{ $quiz->id }}" focusable>
    <div class="max-w-md mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        <div class="p-6 bg-gray-100">
            ...
            <h2 class="mt-4 text-lg font-medium text-center text-gray-900">
                @lang('trad.Are you sure you want to delete this quiz?')
            </h2>
            <p class="mt-2 text-sm text-center text-gray-600">
                @lang('trad.Once this quiz is deleted all of its data will be permanently deleted')
            </p>
            <form method="post" action="{{ route('quiz.delete') }}" class="mt-6">
                @csrf
                @method('DELETE')
