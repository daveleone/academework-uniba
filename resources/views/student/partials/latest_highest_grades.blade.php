...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg hover:shadow-md transition duration-300 ease-in-out">
    <div class="p-6">
        ...
        @if($nonRepeatableQuizzes->count() > 0)
            @foreach($nonRepeatableQuizzes as $quiz)
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex-grow">
                                <div class="flex items-center mb-3">
                                    <a href="{{ route('student.exercises', $quiz->quiz->course_quiz->course->id) }}" class="mr-3 bg-indigo-100 rounded-full p-2 hover:bg-indigo-200 transition-colors duration-200">
                                        <x-heroicon-o-clipboard-document-check class="w-6 h-6 text-indigo-600"/>
                                    </a>
                                    <h3 class="text-xl font-semibold text-gray-900">{{ $quiz->quiz->name }}</h3>
                                </div>
                                <p class="text-sm text-gray-600 mb-2">
                                    <x-heroicon-o-academic-cap class="w-4 h-4 inline mr-1" />
                                    {{ $quiz->quiz->course_quiz->course->course_name }}
                                </p>
                                <div class="flex items-center text-sm text-gray-500">
                                    <x-heroicon-o-calendar class="w-4 h-4 mr-1" />
                                    <span>{{ $quiz->created_at->format('d M, H:i') }}</span>
                                </div>
                            </div>
                            <div class="text-right">
                                <span class="inline-block bg-green-100 text-green-800 text-lg font-semibold px-3 py-1 rounded-full">
                                    {{ $quiz->mark }} / {{ $quiz->maxPoints }}
                                </span>
                            </div>
                        </div>
                        <div class="mt-4 flex justify-end">
                            <a href="{{ route('student.exercises', $quiz->quiz->course_quiz->course->id) }}" class="inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-500 transition-colors duration-200">
            ...
            @endforeach
                    <div class="mt-6">
                        {{ $nonRepeatableQuizzes->appends(['marks' => request()->mark])->links() }}
                    </div>
            </ul>
        @else
